...
void i2c_send(uint8_t data){ // buffers up data to send
  if (USI_BufIdx >= USI_BUF_SIZE) return;         // dont blow out the buffer
  USI_BufIdx++;                                   // inc for next byte in buffer
  USI_Buf[USI_BufIdx] = data;
}

uint8_t i2c_endTransmission(){ // actually sends the buffer
  unsigned char xferOK = false;
  uint8_t errorCode = 0;
...
uint8_t i2c_receive(){ // returns the bytes received one at a time
  USI_LastRead++;     // inc first since first uint8_t read is in USI_Buf[1]
  return USI_Buf[USI_LastRead];
}

uint8_t i2c_available(){ // the bytes available that haven't been read yet
